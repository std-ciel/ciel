...
- Thus, for adding a new reserved keyword, you need to:
  - Add it to the `token_type` enum in `token_type.hpp`.
...
## Operators
- The lexer recognizes various operators defined in `operators_map`. Each operator is mapped to a specific token type in the `token_type` enum.
- For the addition of new operators, you first need to define it in enum class `token_type` in `token_type.hpp`, then add the operator pattern in `operator_map` and also update the `token_to_string_map` in `token_type.cpp`.
...
- If the error is fatal, it will terminate the lexer; otherwise, it will continue.
